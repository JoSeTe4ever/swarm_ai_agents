...
class CommunicationTool(BaseTool):
    """
    This tool enables the Requirements Analyst Agent to communicate with the CEO and UI/UX Designer agents.
    ...
    recipient: str = Field(
        None, description="The recipient of the message or meeting: 'CEO' or 'UI/UX Designer'. Needed for 'send_message' and 'schedule_meeting' actions."
    )
    message: str = Field(
    ...
    def run(self):
        ...
        if self.action == 'send_message':
            if not self.recipient or self.recipient not in ['CEO', 'UI/UX Designer']:
            ...
            # Filter messages for the Requirements Analyst Agent
            received_messages = [msg for msg in messages if msg['recipient'] == 'Requirements Analyst']
